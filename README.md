...
A batteries-included starting point for building **Echo** extensions on Android and Desktop (JVM).
...
```
echo-extension-template/
├── common/                     # Shared Kotlin code (pure JVM, no Android deps)
│   └── src/main/kotlin/…/
│       └── ExtensionBase.kt        ← shared base class (extend this on each platform)
│
├── android/                    # Android APK module
│   └── src/main/kotlin/…/
│       └── MyExtension.kt          ← Android entry-point (edit this)
│
├── desktop/                    # Desktop shadow-JAR module
│   └── src/main/kotlin/…/
│       └── MyExtension.kt          ← Desktop entry-point (edit this)
│
├── buildSrc/                   # Convention plugins & metadata helpers
│   └── src/main/kotlin/
│       ├── ExtensionMetadata.kt    ← data class + Project helper
│       ├── ProguardUtils.kt        ← auto-generates keep rules
│       ├── ProjectConfig.kt        ← Java / JVM target constants
...
├── settings.gradle.kts
└── .github/workflows/build.yml ← CI/CD (build + auto-release on tags)
```
...
```bash
git clone https://github.com/YourName/echo-extension-template.git my-extension
cd my-extension
```
...
Install the APK on your device, or load the JAR into Echo Desktop.
...
- Reads public/private output directories from `Environment`.
- Accesses `Application` context via reflection (no manifest permission needed).
- ProGuard rules are auto-generated by `buildSrc/ProguardUtils.kt` — your extension class is always kept.
...
- Implement echo-common client interfaces directly on `MyExtension` (no imposed base class).
- Uses `System.getProperty("user.home")` for output paths.
- Published to Maven via the `maven-publish` plugin (JitPack-compatible).

### `buildSrc`

Gradle convention plugins that keep build scripts DRY:

- **`ext-convention`** — applies Java/Kotlin source-compatibility settings to every module.
- **`ExtensionMetadata`** — reads `gradle.properties` into a typed data class used by all build scripts.
- **`ProguardUtils`** — generates a `.pro` file that keeps your extension's main class.
